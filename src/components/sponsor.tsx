...
const colors = {
  ...
  textLight: '#ffffff',
  textDark: '#1a1a1a',
  bgGray: '#f4f5f7', // Light gray for inputs matching reference
};
...
const PageWrapper = styled.div`
  ...
  @media (max-width: 768px) {
    padding: 20px 10px;
    align-items: flex-start; /* Better for long scrolling on mobile */
  }
`;
...
const MainCard = styled.div`
  ...
  width: 100%;
  max-width: 1100px;
  height: 85vh; /* Fixed height on desktop for internal scroll */
  background: #ffffff;
  border-radius: 24px;
...
const LeftPanel = styled.div`
  flex: 0 0 45%;
  background-image: url('/assets/jatra-logo.png'); /* Placeholder */
  background-color: ${colors.primaryDarkBlue}; /* Fallback color */
...
const RightPanel = styled.div`
  flex: 1;
  padding: 50px 60px;
  overflow-y: auto; /* Internal Scroll for desktop */
  background: #ffffff;
...
const StyledInput = styled.input`
  ...
  &:focus {
    ...
    box-shadow: 0 4px 12px rgba(0, 135, 217, 0.1);
  }
...
const StyledSelect = styled.select`
  ...
  outline: none;
  cursor: pointer;
  appearance: none; /* Removes default arrow to make it cleaner */
...
const PillButton = styled.button<{ selected: boolean }>`
  ...
  &:hover {
    background-color: ${props => props.selected ? colors.primaryDarkBlue : '#e2e5e9'};
  }
`;
...
const SubmitButton = styled.button`
  ...
  &:hover {
    ...
    box-shadow: 0 10px 25px rgba(255, 54, 0, 0.4);
  }
`;
      ...
      <MainCard>
        ...
        <LeftPanel>
          <LeftContent>
            ...
            {/* <UrgencyBadge>⚡ Limited Partner Slots Available</UrgencyBadge> */}
          </LeftContent>
        </LeftPanel>
        ...
        <RightPanel>
          {!isSubmitted ? (
            <form onSubmit={handleSubmit}>
              ...
              <FormSection>
                ...
                <InputGrid>
                  ...
                  <StyledInput type="text" placeholder="Organization / Brand Name *" required />
                </InputGrid>
              </FormSection>
              ...
              {/* SECTION 3: Type of Collaboration */}
              <FormSection>
                ...
                {/* Hidden input to ensure HTML5 validation if needed, or handle in JS */}
              </FormSection>
              ...
              <FormSection>
                ...
                <InputGrid>
                  <StyledInput type="url" placeholder="Website / Instagram Link" />
                  <StyledInput type="text" placeholder="Approx Contribution Value (₹)" />
                </InputGrid>
                <StyledInput 
                  type="file" 
                  style={{ marginTop: '15px', padding: '11px 18px' }} 
                  accept=".pdf,.png,.jpg,.jpeg"
                />
              </FormSection>

              {/* FINAL CTA */}
              <SubmitButton type="submit">Join the Movement</SubmitButton>
            ...
            <SuccessContainer>
              ...
              {/* Optional secondary contact link
              <p style={{ marginTop: '20px', fontSize: '14px', color: '#888' }}>
                Have urgent questions? <a href="mailto:contact@jatra.com" style={{ color: colors.primaryBlue, fontWeight: 'bold' }}>Contact Us</a>
              </p> 
              */}
            </SuccessContainer>
          )}
